...
L'objectif du produit : développer une application web sur le modèle deun réseau social, dédiée au **netlinking** (mise en relation, gestion de profils, sites et contenus). :contentReference[oaicite:1]{index=1}

## Ce que j'ai fait

### Développement de la V1 (sortie mai 2022)
* **Système de favoris** : ajout d'un mécanisme de mise en favoris des profils (composant Vue + communication avec l'API). :contentReference[oaicite:2]{index=2}
* **Algorithme de placement automatisé** sur une carte type quadrillage (style “bataille navale”) :
  ...
  * possibilité de déplacer les utilisateurs ensuite. :contentReference[oaicite:3]{index=3}
* **Pagination générique** : création d'un composant Vue réutilisable pour paginer et classer des listes renvoyées par l'API (notamment pages d'un site associé à un utilisateur). :contentReference[oaicite:4]{index=4}
* **Librairie JavaScript (npm)** : développement d'une lib de placement automatisé de lien :
  ...
  * gestion d'occurrences multiples (placements successifs sans décaler les index). :contentReference[oaicite:5]{index=5}
...
* Mise en place d'un **assistant multi-étapes** (front Vue + back) pour aider les nouveaux utilisateurs à configurer correctement leur compte :
  ...
  * redirection vers la carte pour lancer le placement automatique. :contentReference[oaicite:6]{index=6}

### Développement de la V2 (sortie décembre 2023)
* **Système de chat** :
  * back + front,
  * utilisation de **WebSockets**,
  * composants Vue pour gérer channels, tri/affichage des messages. :contentReference[oaicite:7]{index=7}
* **Moteur de recherche** :
  ...
  * exploitation des mots-clés pour enrichir la recherche. :contentReference[oaicite:8]{index=8}
* **Refonte / réorganisation du produit** :
  * échanges avec le fondateur,
  * découpage en tickets page par page,
  * déplacement/réorganisation de fonctionnalités et écrans. :contentReference[oaicite:9]{index=9}
...
* Travail “qualité” : composants réutilisables, logique robuste (pagination, occurrences, placement). :contentReference[oaicite:10]{index=10}
